...
const IGNORED_DIRS = new Set(['node_modules', 'dist', '.git', '.gemini', 'scripts', 'tests', 'public', 'maps_db', 'coverage']);

/**
 * Recursively flatten nested JSON objects into dot-notated key-value pairs.
 */
...
function extractKeysFromCodebase(dir) {
  ...
  function scanDirectory(currentDir) {
    ...
    for (const entry of entries) {
      ...
      } else if (/\.(html|hbs|handlebars|js)$/.test(entry.name)) {
...
/**
 * Scan a single file for i18n keys using regex patterns matching maps's i18n engine.
 */
function scanFile(filePath, keysSet) {
  const content = fs.readFileSync(filePath, 'utf-8');

  // 1. Explicit data-i18n-* attributes with attribute values:
  // e.g. data-i18n="Key", data-i18n-title="Key", data-i18n-placeholder="Key", data-i18n-aria-label="Key", data-i18n-html="Key", etc.
  const attrRegex = /data-i18n(?:-[a-zA-Z0-9_-]+)?=["']([^"']+)["']/g;
  let match;
  ...
  // 2. Elements with exact data-i18n attribute containing inner text:
  ...
  // 3. TranslationController.t('Key'), this.t('Key'), getTranslation('Key'), or t('Key') calls in JavaScript files
  const jsTranslationRegex = /\b(?:TranslationController\s*\.\s*t|this\s*\.\s*t|getTranslation|t)\s*\(\s*['"`]([^'"`]+)['"`]/g;
  while ((match = jsTranslationRegex.exec(content)) !== null) {
    const val = match[1].trim();
    if (val && !val.includes('${')) {
      keysSet.add(val);
    }
...
function main() {
  ...
  for (const file of langFiles) {
    ...
    try {
      ...
      languages[langCode] = flattenKeys(parsed);
    } catch (err) {
