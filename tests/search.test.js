...
import { SearchController } from '../js/search/index.js';
...
describe('SearchController', () => {
  ...
  describe('Recent Searches & Base Suggestions', () => {
    ...
    it('renders Home address and other saved places as pills, and previous searches underneath', () => {
      ...
      expect(chips[0].getAttribute('label')).toBe('Home: Via Roma 1');
      expect(chips[0].getAttribute('icon')).toBe('home');
      expect(chips[1].getAttribute('label')).toBe('Arena di Verona');
      expect(chips[2].getAttribute('label')).toBe('Castelvecchio');
    ...
    it('clicking a saved place pill flies to location and sets search bar value', () => {
      MarkerController.customMarkers = [
        { id: '1', title: 'Arena di Verona', lat: 45.439, lng: 10.994 }
      ];
      MapService.getHomeAddress.mockReturnValue(null);
      ...
      expect(MapService.flyTo).toHaveBeenCalledWith([10.994, 45.439], 15);
      expect(document.getElementById('search-bar').value).toBe('Arena di Verona');
      expect(HUDController.setState).toHaveBeenCalledWith('place-details', expect.objectContaining({
